...
# for prediction first we will use bag of words techniques
...
p1 = threading.Thread(target=build_confusion_matrix, args=(nb_pipeline,))
p2 = threading.Thread(target=build_confusion_matrix, args=(logR_pipeline,))
p3 = threading.Thread(target=build_confusion_matrix, args=(svm_pipeline,))
p4 = threading.Thread(target=build_confusion_matrix, args=(random_forest,))
...
# K-fold cross validation for all classifiers
# Creating processes for n-grams cross validation
p5 = threading.Thread(target=build_confusion_matrix, args=(nb_pipeline_ngram,))
p6 = threading.Thread(target=build_confusion_matrix, args=(logR_pipeline_ngram,))
p7 = threading.Thread(target=build_confusion_matrix, args=(svm_pipeline_ngram,))
p8 = threading.Thread(target=build_confusion_matrix, args=(random_forest_ngram,))
...
# Towards printing classification report
...
print("\nCompleted saving, towards plotting curve")
...
def plot_learning_curve(pipeline, title):
    ...
    train_sizes, train_scores, test_scores = learning_curve(pl, X, y, n_jobs=-1, cv=cv,
                                                            train_sizes=np.linspace(.1, 1.0, 5), verbose=True)
...
# below command will plot learing curves for each of the classifier
